...
def waveform_to_log_mel_spectrogram_patches(waveform, params):
    """Compute log mel spectrogram patches of a waveform."""
    with tf.name_scope('log_mel_features'):
        # waveform shape: [batch_size, num_samples] 또는 [num_samples]

        # 입력이 1D 텐서인 경우 배치 차원 추가
        if len(waveform.shape) == 1:
            waveform = tf.expand_dims(waveform, axis=0)  # [1, num_samples]
        ...
        magnitude_spectrogram = tf.abs(tf.signal.stft(
            ...
            fft_length=fft_length,
            pad_end=False))
        # magnitude_spectrogram shape: [batch_size, num_frames, num_spectrogram_bins]
        ...
        mel_spectrogram = tf.tensordot(
            magnitude_spectrogram, linear_to_mel_weight_matrix, axes=[2, 0])
        # mel_spectrogram shape: [batch_size, num_frames, params.mel_bands]

        log_mel_spectrogram = tf.math.log(mel_spectrogram + params.log_offset)
        # log_mel_spectrogram shape: [batch_size, num_frames, params.mel_bands]

        # 패치 생성
        spectrogram_hop_length_samples = int(
            round(params.sample_rate * params.stft_hop_seconds))
        spectrogram_sample_rate = params.sample_rate / spectrogram_hop_length_samples
        patch_window_length = int(
            round(spectrogram_sample_rate * params.patch_window_seconds))
        patch_hop_length = int(
            round(spectrogram_sample_rate * params.patch_hop_seconds))

        # 프레임을 패치로 변환
        features = tf.signal.frame(
            log_mel_spectrogram,
            frame_length=patch_window_length,
            frame_step=patch_hop_length,
            axis=1)  # 프레임 축(axis=1)을 따라 프레임화
        # features shape: [batch_size, num_patches, patch_window_length, params.mel_bands]
        ...
        #tf.print('log_mel_spectrogram shape:', tf.shape(log_mel_spectrogram))
...
def pad_waveform(waveform, params):
  ...
  # need at least one patch window length of waveform plus enough extra samples
  # to complete the final STFT analysis window.
  waveform = tf.reshape(waveform, [-1, tf.shape(waveform)[-1]])  # [batch_size, num_samples]
  ...
  padded_waveform = tf.pad(waveform, [[0, 0], [0, num_padding_samples]],
                             mode='CONSTANT', constant_values=0.0)
  #tf.print('waveform shape after padding:', tf.shape(padded_waveform))
  return padded_waveform
